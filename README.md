...
| 模块 | 用途 | 当前状态 |
| --- | --- | --- |
| 字体库 | 汇集并检查各游戏中的架空文字字体 | 29 个内置字体实例在启动时自动载入，并以只读库存显示；按游戏目录待实现 |
| 字形查阅 | 对照普通输入字符与架空字形 | 使用独立字体选择器，不受字体管理页影响；正反查目录待实现 |
...
当前开发分支携带由 11 个 `.glyphs` 源可重复构建的 29 个静态 TTF。应用启动时会把它们载入当前进程，无需先安装到 Windows，也无需选择外部字体。字体管理页只显示库存和加载诊断；字形查阅页独立选择要查阅的字体，并以普通输入字符而非 Unicode 技术编号作为对照。当前列表后续会由按游戏与文字体系组织的目录替换。
